...
import transforms
# import torchvision
from data_edge2 import fashion_MNIST_AG, fashion_MNIST
import icpnet_model
import time
import os
...
def AG_test(illname):
    # inference stage
    length = len(test_AG_fashion_MNIST_dataloader)
    total_imgs = test_AG_fashion_MNIST_dataset.__len__()
    t_time = 0.
    correct_top1 = 0
    correct_top5 = 0
    correct = 0
    ...
    net.eval()
    with torch.no_grad():
        for i, data in enumerate(test_AG_fashion_MNIST_dataloader, start=0):

            b, _, h, w = data['images'].shape
            x_2 = torch.zeros((b, 128, h//4, w//4), device='cuda:0')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./icpnet'):
        os.makedirs('./icpnet')
    name = cfgs['name']['fashion_MNIST']
    log = open('./icpnet/'+'1train_' + name + '#########.txt', 'a+')
    name_ag = cfgs['name']['fashion_MNIST']
    ...
    trans = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[[0.2910], [0.]], std=[[0.3126], [1.]])
    ])

    test_trans = transforms.Compose([
        transforms.Resize((224, 224), interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.ToTensor(),
        transforms.Normalize(mean=[[0.2910], [0.2910]],std=[[0.3126], [0.3126]])])

    AG_fashion_MNIST_trans = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.2910], std=[0.3126])])

    # MNIST data
    train_dataset = fashion_MNIST(root=cfgs['dataset'], flag='train', transform=trans)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=cfgs['batch_size'], shuffle=True, pin_memory=True, num_workers=6)
    ...
    for epoch in range(start_epoch + 1, cfgs['max_epoch']):
        ...
        # # ##########test && plot AG_fashion_MNIST@top1 curve
        for i in range(len(ill_name)):
            test_AG_fashion_MNIST_dataset = fashion_MNIST_AG(root=cfgs['dataset'], flag='test', transform=AG_fashion_MNIST_trans, ill_name=ill_name[i])
            test_AG_fashion_MNIST_dataloader = torch.utils.data.DataLoader(test_AG_fashion_MNIST_dataset, batch_size=20, shuffle=False, num_workers=2)
            AG_test(ill_name[i])
